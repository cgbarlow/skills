...
# woolies-shopper (v0.2.2 — exception resolver)
...
**This skill is NOT the entry point for the full weekly shop.** That's `./scripts/shop.sh` in the same skill directory — a master bash orchestrator that runs three phases: (1) headless `claude -p` that OCRs the newest meal-plan photo in the current directory, pauses for the user to confirm the parse, then creates the meal plan + aggregation in Iris, (2) pure-bash bulk-add against cached SKUs, (3) this skill for exception resolution. If a user types "do my Woolies shop" into a bare Claude session, your first reply should point them at `shop.sh` from the terminal rather than running the whole workflow inside Claude — the orchestrator is faster and cheaper.
